/**
 * passwordsNotMatch.js
 *
 * A custom response.
 *
 * Example usage:
 * ```
 *     return res.passwordsNotMatch();
 *     // -or-
 *     return res.passwordsNotMatch(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
 *     exits: {
 *       somethingHappened: {
 *         responseType: 'passwordsNotMatch'
 *       }
 *     }
 * ```
 *
...
module.exports = function passwordsNotMatch(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.passwordsNotMatch()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.passwordsNotMatch()` called with an Error:', optionalData);
